...
int main() {
    ...
    cout << c4.aggiungi_mina(0, 1) << ' ';          // queste mine devono essere aggiunte perché la
    cout << c4.aggiungi_mina(1, 1) << ' ';          // precedente scopri è stata invocata su un gioco
    ...
    CampoMinato c3(c2);
    c3.scopri(-1, 2);
    c3.scopri(0, 3); // le scopri devono fallire perché fuori bound
    cout << c3 << endl;

    cout <<"Altri test della aggiungi_mina:"<<endl;
    cout << c3.aggiungi_mina(2, 2) << ' '; // La mina deve comunque esser piazzata perché le scopri hanno fallito
